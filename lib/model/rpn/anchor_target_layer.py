...
class _AnchorTargetLayer(nn.Module):
  ...
  def forward(self, input):
    ...
    shifts = shifts.contiguous().type_as(rpn_cls_score).float() # shifts is the center coordinates for the anchors
    ...
    all_anchors = all_anchors.view(K * A, 4) # all anchors coordinates for a single image
    ...
    labels = gt_boxes.new(batch_size, inds_inside.size(0)).fill_(-1) # new_full, new_zeros
