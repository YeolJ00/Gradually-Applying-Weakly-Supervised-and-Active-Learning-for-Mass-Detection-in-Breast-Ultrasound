...
class _RPN(nn.Module):
    ...
    def __init__(self, din):
        ...
        self.RPN_Conv = nn.Sequential(nn.Conv2d(self.din, 512, kernel_size = 3, stride = 1, padding = 1, bias=True), nn.ReLU())

        # define bg/fg classifcation score layer
        self.nc_score_out = len(self.anchor_scales) * len(self.anchor_ratios) * 2 # 2(bg/fg) * 9 (anchors)
        self.RPN_cls_score = nn.Conv2d(512, self.nc_score_out, kernel_size = 1, stride = 1, padding =  0, bias= True)

        # define anchor box offset prediction layer
        self.nc_bbox_out = len(self.anchor_scales) * len(self.anchor_ratios) * 4 # 4(coords) * 9 (anchors)
        self.RPN_bbox_pred = nn.Conv2d(512, self.nc_bbox_out, kernel_size = 1, stride =  1, padding = 0, bias = True)
