...
class _ProposalLayer(nn.Module):
  ...
  def __init__(self, feat_stride, scales, ratios):
    ...
    # _anchors is a list of anchors [[x,y,x,y],[x,y,x,y],[x,y,x,y],[x,y,x,y]]
  ...
  def forward(self, input):
    ...
    shift_x = np.arange(0, feat_width) * self._feat_stride # shift_x coordinates
    ...
    anchors = self._anchors.view(1, A, 4) + shifts.view(K, 1, 4) # broadcasts and generates anchor coordinates for all loactions
    anchors = anchors.view(1, K * A, 4).expand(batch_size, K * A, 4) # duplicate for all batches (batch, K*A, 4)

    # Transpose and reshape predicted bbox transformations to get them
    # into the same order as the anchors:

    bbox_deltas = bbox_deltas.permute(0, 2, 3, 1).contiguous() # (batch, anchor*4, H, W) --> (batch, H, W, anchor*4)
    bbox_deltas = bbox_deltas.view(batch_size, -1, 4)      # --> (batch, H*W*anchor, 4)

    # Same story for the scores:
    scores = scores.permute(0, 2, 3, 1).contiguous() # (batch, anchor, H, W) --> (batch, H, W, anchor)
    scores = scores.view(batch_size, -1)         # --> (batch, H*W*anchor) are these foreground prob?
