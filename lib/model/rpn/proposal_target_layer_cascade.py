...
class _ProposalTargetLayer(nn.Module):
    """
    Assign ground-truth targets to object detection proposals. Produces proposal
    ...
    def forward(self, all_rois, gt_boxes, num_boxes):
        ...
        # bbox_inside_weights: (batch, anchor, 4)
    ...
    def _sample_rois_pytorch(self, all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
        ...
        num_boxes_per_img = overlaps.size(2) # gt_boxes

        offset = torch.arange(0, batch_size)*gt_boxes.size(1)
        offset = offset.view(-1, 1).type_as(gt_assignment) + gt_assignment # max overlap gt

        # changed indexing way for pytorch 1.0
        labels = gt_boxes[:,:,4].contiguous().view(-1)[(offset.view(-1),)].view(batch_size, -1)
        # (batch, 1) contains max overlap gt_box class
